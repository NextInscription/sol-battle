...
export type ActiveBattle = {
  ...
  gameTypeDisplay: 'Flip Card Game' | 'Dice Roll Battle';
  state: BattleState;
  stateDisplay: string;
...
function parseBattleAccount(data: Buffer): any {
  ...
  const gameTypeTag = data.readUInt8(offset);
  const gameType = gameTypeTag === 0 ? 'CoinFlip' : 'DiceRoll';
  offset += 1;
  ...
  for (let i = 0; i < battleAccounts.length; i++) {
    try {
      ...
      result.push({
        ...
        gameTypeDisplay: parsed.gameType === 'CoinFlip' ? 'Flip Card Game' : 'Dice Roll Battle',
